...
Assets/
└── _Project/
    ├── _Project.asmdef          # Assembly definition for the project
...
Features contain only code and feature-specific resources. **All ScriptableObject assets** live in `Catalog/`, organised by feature — mirroring the structure of `Core/` and `Features/`. This includes Configurations, Definitions, Handlers, and Loaders. The C# scripts that define these ScriptableObjects live in the feature; the instantiated `.asset` files live in Catalog.
...
| | Repository | CRUD operations on local persistent storage                                                     | Class |
...
**Controllers** coordinate Unity components, injected services, and event systems on a GameObject. They manage lifecycle (`OnEnable`, `OnDisable`), own subscriptions, and expose a public API for other systems to interact with. Controllers are always MonoBehaviours.
...
| **API** | Feature-specific public methods | Feature-specific Update method |
...
**Repositories** handle CRUD operations on local persistent storage. They own the persistence boundary and expose domain-friendly methods rather than raw storage calls. The pattern is the same regardless of storage engine; only the implementation changes.

**Registries** are the runtime counterpart to Repositories. Where a Repository persists data to storage, a Registry tracks what's alive and accessible right now — active entities, spawned objects, loaded resources — typically keyed by their ID. Registration, retrieval, and deregistration of active objects.
...
└── Console/
    ...
    │   ├── ConsoleLog.cs
    │   └── ConsoleCategory.cs
    ├── Loaders/                    # Boot-time loader ScriptableObject scripts
    │   └── ConsoleLoader.cs
...
The `Catalog/` folder mirrors the feature structure. Each feature's ScriptableObject assets are grouped together, just as their scripts are grouped in `Features/`:
...
**New Feature** and **New Core Package** prompt for a name and scaffold the full package structure — the package entry point, Components folder with a Model and Presenter, Contracts folder with the service interface, MonoBehaviours folder with a View, and a Loader. All classes are correctly named, sealed, and wired with constructor injection. The corresponding `Catalog/` folder is created with the Loader asset.
