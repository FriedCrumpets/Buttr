...
namespace Buttr.Tests.Editor.DIContainer {
    public sealed class DITContainerTests {
        ...
        [Test] public void GetConcreteService_WhenRegistered_IsRetrievable() {
            m_DIBuilder.AddSingleton<Service1>(0);
            var container = m_DIBuilder.Build();
            var service = container.Get<Service1>(0);

            // service should be retrievable
            Assert.IsTrue(service is not null);
        }

        [Test] public void GetAbstractService_WhenConcreteIsRegistered_IsRetrievable() {
            m_DIBuilder.AddSingleton<Service1>(0);
            var container = m_DIBuilder.Build();
            var service = container.Get<IService1>(0);

            // service should be retrievable via its interface
            Assert.IsTrue(service is not null);
        }

        [Test] public void TryGetService_WhenRegistered_ReturnsTrueAndValueIsService() {
            m_DIBuilder.AddSingleton<Service1>(0);
            var container = m_DIBuilder.Build();
            var located = container.TryGet<Service1>(0, out var service);

            // service should be retrievable
            Assert.IsTrue(located is true && service is not null);
        }
