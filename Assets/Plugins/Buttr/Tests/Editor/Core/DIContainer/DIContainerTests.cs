...
namespace Buttr.Tests.Editor.DIContainer {
    public sealed class DIContainerTests {
        ...
        private interface IService1 { int Test { get; } }
        ...
        private Core.DIBuilder m_DIBuilder;

        [SetUp]
        public void Setup() {
            m_DIBuilder = new Core.DIBuilder();
        }
        ...
        [Test] public void GetConcreteHiddenService_WhenNotRegistered_ReturnsDefault() {
            var container = m_DIBuilder.Build();
            var service = container.Get<Service1>();

            // if not registered via Hidden the service should be null
            Assert.IsTrue(service is null);
        }

        [Test] public void GetConcreteHiddenService_WhenRegistered_Throws() {
            m_DIBuilder.Hidden.AddSingleton<Service1>();
            var container = m_DIBuilder.Build();
        ...
        [Test] public void GetAbstractHiddenService_WhenNotRegistered_ReturnsDefault() {
            var container = m_DIBuilder.Build();
            var service = container.Get<IService1>();
        ...
        [Test] public void GetAbstractHiddenService_WhenRegistered_Throws() {
            m_DIBuilder.Hidden.AddSingleton<IService1, Service1>();
            var container = m_DIBuilder.Build();
        ...
        [Test] public void TryGetHiddenService_WhenNotRegistered_ReturnsDefault() {
            var container = m_DIBuilder.Build();
            var located = container.TryGet<IService1>(out var service);

            // if not registered via Hidden the service should not be found
            Assert.IsTrue(located is false && service is null);
        }
        ...
        [Test] public void GetService_WithHiddenDependencies_ReturnsServiceWithDependenciesNotNull() {
            m_DIBuilder.AddSingleton<IService0, Service0>();
            m_DIBuilder.Hidden.AddSingleton<Service1>();
            m_DIBuilder.AddSingleton<IService2, Service2_1>();
            var container = m_DIBuilder.Build();

            // Hidden services should still be injectable as dependencies
            Assert.IsTrue(container.Get<IService2>().Validate());
        }
